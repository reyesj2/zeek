...
public:
	...
	explicit BrokerDataView(broker::data* value) noexcept : value_(value)
		{
	 ...
	 */
	friend std::string to_string(const BrokerDataView& data)
		{
 ...
 */
template <typename... Args> [[nodiscard]] inline bool IsCount(BrokerDataView arg, Args&&... args)
	{
 ...
 */
template <typename... Args> [[nodiscard]] inline auto ToCount(BrokerDataView arg, Args&&... args)
	{
...
public:
	...
	explicit BrokerListView(broker::vector* values) noexcept : values_(values)
		{
	 ...
	 * @pre Size() > 0
	 */
	[[nodiscard]] BrokerDataView Front() const
		{
		return BrokerDataView{std::addressof(values_->front())};
		}
	 ...
	 * @pre Size() > 0
	 */
	[[nodiscard]] BrokerDataView Back() const
		{
		return BrokerDataView{std::addressof(values_->back())};
		}
...
public:
	 ...
	 */
	[[nodiscard]] static RecordValPtr ToRecordVal(const ValPtr& value)
		{
		return ToRecordVal(value.get());
		}
...
public:
	 ...
	 */
	template <typename T> void AddCount(T value)
		{
	 ...
	 */
	template <class... Ts> void AddList(Ts&&... values)
		{
