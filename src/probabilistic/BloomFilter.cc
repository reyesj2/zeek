...
std::unique_ptr<BloomFilter> BloomFilter::Unserialize(BrokerDataView data)
	...
	switch ( v[0].ToCount() )
		{
		case Basic:
			bf.reset(new BasicBloomFilter());
			break;

		case Counting:
			bf.reset(new CountingBloomFilter());
			break;
...
std::optional<BrokerData> BasicBloomFilter::DoSerialize() const
	{
...
std::optional<BrokerData> CountingBloomFilter::DoSerialize() const
	{
	return cells->Serialize();
	}
